const fetchData = (nodes, edges, graph, that) => {
	...
	return new Promise((resolve, reject) => {
		const p1 = fetch(window.location.href + 'pod');
		const p2 = fetch(window.location.href + 'service');
		const p3 = fetch(window.location.href + 'ingress');
		const p4 = fetch(window.location.href + 'daemonset')
